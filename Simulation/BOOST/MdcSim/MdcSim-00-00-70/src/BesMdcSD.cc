...
:BesSensitiveDetector(name)
{
    ...
    //yzhang FIXME
    ////get MdcCalibFunSvc
    //sc= Gaudi::svcLocator()->service("MdcCalibFunSvc",mdcCalFunSvc);
    //if (!sc.isSuccess()) std::cout <<" MdcCal::Could not open Cal Service"<<std::endl;
...
void BesMdcSD::Initialize(G4HCofThisEvent* HCE)
{
  ...
  for(i=0; i<43;i++){
    //for(j=0;j<288;j++)
    for(j=0;j<400;j++){//yzhang changed for CDC
      hitPointer[i][j]=-1;
      truthPointer[i][j]=-1;
    }
  }
...
{
  if(SDenter==2)return false;//yzhang 2014-04-18 multi turn
...
//G4cout<<"hitPosition "<<hitPosition<<" driftD "<<driftD<<G4endl;
  ...
  newHit->SetEdep(gTrack->GetTrackLength()*1000);
  ...
  else if(SDlastLayer==layerId&&cellId==SDlastCell)
  {
      ...
      if (driftT < preDriftT) {   
          ...
          (*hitsCollection)[pointer]->SetEdep(gTrack->GetTrackLength()*1000);
  ...
  if(truthCollection){
    if(g4TrackID==trackID){ //This track is the primary track & will appear in MC truth
      //std::cout<<__FILE__<<"   "<<__LINE__<<" layerId  "<<layerId<<" cellId "<<cellId<<" SDlastLayer "<<SDlastLayer<<" SDlastCell "<<SDlastCell<<truthPointer[layerId][cellId]<<" truthID "<<truthID<<std::endl;
      G4int pointer=truthPointer[layerId][cellId];  
      if(pointer==-1){
        BesMdcHit* truthHit = new BesMdcHit();
	//std::cout<<__FILE__<<"   "<<__LINE__<<" new truthHit  "<<std::endl;
        truthHit->SetTrackID  (truthID);
        truthHit->SetLayerNo(layerId);
        truthHit->SetCellNo(cellId);
	//std::cout<<__FILE__<<"   "<<__LINE__<<" truthCollection  "<<layerId<<" "<<cellId<<std::endl;
        truthHit->SetEdep     (gTrack->GetTrackLength()*1000);
	  ...
	  if(layerId==SDlastLayer&&cellId==SDlastCell){ 
	    ...
	    edepTemp=(*truthCollection)[pointer]->GetEdep();
            (*truthCollection)[pointer]->SetEdep(gTrack->GetTrackLength()*1000);
      ...
      //yzhang add a truth hit to save momentum of this hit 2014-04-28 
      BesMdcHit* truthHit_mom = new BesMdcHit();
      //std::cout<<__FILE__<<"   "<<__LINE__<<" new truthHit for mom "<<std::endl;
      truthHit_mom->SetPosFlag(-999);//set flag -999
      truthHit_mom->SetTrackID  (truthID);
      truthHit_mom->SetLayerNo(layerId);
      truthHit_mom->SetCellNo(cellId);
      truthHit_mom->SetPos      (outP);//put momentum of out point to position
      truthCollection->insert(truthHit_mom);
      //zhangy
    }
  }
  if(layerId==0&&SDlastLayer>0)SDenter++;//yzhang 2014-04-18 muti turn
...
G4double BesMdcSD::Distance(G4int layerId, G4int cellId, G4ThreeVector pointIn, G4ThreeVector pointOut,G4ThreeVector& hitPosition,G4double& transferT)
{
  ...
  //yzhang should get transfer time on wire from calibration
  transferT = 0;
  //G4double halfLayerLength=mdcGeomSvc->Layer(layerId)->Length()/2.;
