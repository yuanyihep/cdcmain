...
void BesMdcDigitizer::Digitize(){
	...
	//initialize 
	for(G4int i=0; i<43;i++){
		//for(G4int j=0;j<288;j++)//yzhang 2014-04-11 for CDC
		for(G4int j=0;j<400;j++){
			digiPointer[i][j]=-1;
		}
	}
	...
	if(THC){
		...
		for(G4int i=0;i<NHits;i++){
			...
			*/
			...
			//std::cout<<__FILE__<<"   "<<__LINE__<<" new digi  "<<layerId<<" "<<cellId<<std::endl;
