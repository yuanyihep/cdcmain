...
{ 
  ...
  if(HCID>0)
  {
    ...
    if(n_hit>0)
    {
      ...
      {
        hit = (*HC)[i];
        unsigned int scinNum, barrel_ec, layer = 0;
        scinNum = hit->GetScinNb();
        barrel_ec = hit->GetPartId();    

	//std::cout << "BesMcTruthWriter        PartID | scinNum  "  <<  barrel_ec  << "  |  "   << scinNum  << std::endl;
	//std::cout << "BesMcTruthWriter        is_barrel(barrel_ec)    "  <<  TofID::is_barrel(barrel_ec)  << std::endl;
	...
	if(barrel_ec==0 || barrel_ec==1 || barrel_ec==2 )   //Old-Tof 
	  ...
	  {
	    int help = BesTofDigitizerEcV4::Calculate_Readoutstrip_number_continuum(hit->GetPos().x()*mm,hit->GetPos().y()*mm, barrel_ec, scinNum);
	    scinNum = BesTofDigitizerEcV4::Produce_unique_identifier(scinNum, help);
	    //std::cout << "BesMcTruthWriter        Readoutstrip "  <<  help  << std::endl;
	    //std::cout << "BesMcTruthWriter        unique Identifier    "  <<  scinNum  << std::endl;
